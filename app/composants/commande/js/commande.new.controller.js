...
function($scope,$state,DrugStoreService,CommandeService,ProduitService,DailySaleService,
     ...
     async.parallel({
       ...
       addCommande:function(callback){
        ...
        DailySaleService.getCurrentDailySale(function(currentDailySale){
          ...
          CommandeService.create($scope.commande,
            (rep)=>{
              // update dailySale.
        ...
        if (err){
          ...
          $state.go("home",{"selectedMenu":MENU.COMMANDE});
           DrugStoreService.succesRequest(MessageManager.M27);
        }
